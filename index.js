...
require('./config')
const { default: clientConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function start() {
    const client = clientConnect({
        ...
        browser: ['Octave Official Multi Device','Safari','1.0.0'],
        auth: state
    })
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./octave")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    // Get Profile Picture Group
       ...
       ppgc = 'https://telegra.ph/file/26bfff419cb140cf3195f.jpg'
       }
    ...
    client.sendButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        var template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        "hydratedContentText": text,
